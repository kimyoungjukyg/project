...
function getSchedule() {
	...
	$.ajax({
		...
		url : '/member/calendar/getSchedule',
		type : 'POST',
		data : data,
		...
		success : function(responseData, status) {
					...
					for(var i=0;i<responseData.data.length;i++){
							responseData.data[i].start = responseData.data[i].starttime;
							responseData.data[i].end = responseData.data[i].endtime;
...
function addSchedule() {
	...
	$.ajax({
		url : '/member/calendar/addSchedule',
		type : 'POST',
		data : params,
		contentType : 'application/x-www-form-urlencoded; charset=UTF-8',
		dataType : 'json',
